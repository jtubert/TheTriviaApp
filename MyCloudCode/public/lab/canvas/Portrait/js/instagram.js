...
com.jtubert.instagram = function(){
	...
	this.init = function(obj){		
		...
		if(window.location.href.indexOf("http://localhost") == 0){
			$("#login").append("<a href='https://instagram.com/oauth/authorize/?client_id=10b957025e7d4173b00207cbdec99360&redirect_uri=http://localhost:8888/git/canvas/portrait/?mode=instagram&type="+obj.type+"&response_type=token'>Add instagram images</a>");
		}else{
			$("#login").append("<a href='https://instagram.com/oauth/authorize/?client_id=1e1db18d520245f4a4537f2d85438cdb&redirect_uri=http://labs.jtubert.com/canvas/Portrait?mode=instagram&type="+obj.type+"&response_type=token'>Add instagram images</a>");
		}		
		...
		$.ajax({
		  ...
		  error: function(jqXHR,error, errorThrown) {  
		               if(jqXHR.status && jqXHR.status==503){
		                    alert(jqXHR.responseText + " - Please refresh page."); 
		               }else{
	...
	this.userMediaRecentSuccess = function(obj,divName){
		...
		if(self.limit){
			len = (obj.data.length < self.limit)?obj.data.length:self.limit;
		}
		...
		for(var i=0;i<len;i++){			
			...
			*/
			...
			$("#holder").append("<img src='/proxied_image?image_url="+thumbURL+"'/>");
			
			$("#loading").text("Loading all your Instagram pictures..."+$("#holder").find("img").length);
		}	
		...
		if(next_url){
			if(self.limit){
				portrait.allInstagramImagesLoaded(self.userPhotoURL);
			}else{
