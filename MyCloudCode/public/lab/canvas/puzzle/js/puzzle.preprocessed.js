...
com.jtubert.Puzzle = function() {
    ...
    self.init = function(url,_usePhpProxy) {
		...
		if(com.jtubert.Puzzle.Utils.getUrlVars().uid)
			self.addMultiplayerCapabilities();
    };

	self.addMultiplayerCapabilities = function(){
		console.log(XVIcubes.User.ACTIVE_USER_ID);
		
		XVIcubes.Event.registerHandler("UserJoinEvent", self.onEvent);
		XVIcubes.Event.registerHandler("UserLeaveEvent", self.onEvent);
		XVIcubes.Event.registerHandler("ResetEvent", self.onEvent);
		XVIcubes.Event.registerHandler("StateChangeEvent", self.onEvent);
		//XVIcubes.Event.unregisterHandler(eventName)
		...
		//XVIcubes.Leaderboard.postScore(object, callback)		
		//XVIcubes.Leaderboard.getScore(callback)
	}
	
	self.resetGame = function(){
		XVIcubes.Game.reset();

		XVIcubes.Event.registerHandler("ResetEvent", function(event) {
		       self.location.reload();
		});
	}
	
	self.setState = function(stateObj){
		XVIcubes.Game.setState(stateObj, function(state) {
			console.log("setState");     
			console.log(state);
		});
	}
	
	self.onEvent = function(eventObject){
		//console.log("-------------------------------------");
		//console.log(eventObject.event-type);
		//console.log(eventObject);
		//console.log(eventObject.data.state+"-------------------------------------");
		
		if(eventObject.data.switchTurns){
			self.switchTurn();
		}
		
		if(eventObject.data.state == "pieceWasDragged"){
			for(var i=0;i<eventObject.data.pieces.length;i++){
				self.move(eventObject.data.pieces[i].piece_id,eventObject.data.pieces[i].piece_x,eventObject.data.pieces[i].piece_y);
			}
		}else if(eventObject.data.state == "puzzleCompleted"){
	...
	self.getUserInfo = function(){
		var uid = XVIcubes.User.ACTIVE_USER_ID;
		XVIcubes.User.getUserInfo(uid, function(userInfo) {		    
		    console.log(userInfo.first_name, userInfo.last_name, userInfo.profile_pic_small);		    
		});
	}
	
	self.getUsers = function(){
		XVIcubes.User.getUsers(function(users) {
			usersArray = users;
			
			for(var i=0;i<users.length;i++){
				var playerDiv = "<div id='user"+users[i].uid+"'><img src='"+users[i].profile_pic_small+"'></img> "+users[i].first_name+" "+users[i].last_name+"</div>";
	...
	self.switchTurn = function(){
		...
		currentPlayerID = usersArray[currentPlayer].uid;
		console.log("currentPlayerID: "+currentPlayerID);
		
		$("#user"+usersArray[currentPlayer].uid).css("background-color","red");
		$("#user"+usersArray[otherplayer].uid).css("background-color","white");
		
		if(currentPlayerID == XVIcubes.User.ACTIVE_USER_ID){
	...
	self.isMyTurn = function(){
		return (currentPlayerID == XVIcubes.User.ACTIVE_USER_ID);
	}
    ...
    self.createPuzzle = function(img) {
		...
		//change other players puzzle
		self.setState({state:"pieceWasDragged",pieces:arr,switchTurns:false});
    ...
    self.onMouseUp = function(e) {
        ...
        if (dropTarget.id.indexOf("piece") !== -1) {
			...
			self.setState({state:"pieceWasDragged",pieces:arr,switchTurns:true});
        ...
        if (self.isPuzzleCompleted()) {
			...
			self.setState({state:"puzzleCompleted",switchTurns:false});
